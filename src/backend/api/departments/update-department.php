...
try {
    pushLog("Initializing Update Department");
    $database = connectDB();
    ...
    $head = $data["head"];
    $query = "UPDATE TABLE departments(name, description, head) VALUES  ('$name', '$description', '$head')";
    pushLog(
        "<div>
                <p>Updating Department <b>$name</b> with <b>Description</b>:   <i>$description</i></p>
                <p>And Empty parameters id and department</p>
            </div>"
    );
    mysqli_query($database, $query);
    pushLog("Department Updated Successfully.");
