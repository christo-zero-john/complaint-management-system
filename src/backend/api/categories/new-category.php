...
try {
    pushLog("Initializing Create New category");
    $database = connectDB();
    ...
    if (categoryExists($name)) {
        ...
        pushLog(
            "<div>
                <p>Creating New category <b>$name</b> with <b>Description</b>:   <i>$description</i></p>
                <p>And Empty parameter id</p>
            </div>"
        );
        mysqli_query($database, $query);
        pushLog("category Created Successfully.");
    }
...
function categoryExists($name)
{
    ...
    pushLog("Checking whether Category already exists");
    if (mysqli_num_rows($result) > 0) {
        pushLog("Category already exists");
        return 1;
    } else {
        pushLog("Category does not exists");
        return 0;
    }
}
